...
def vacancy_detail(request, pk):
	...
	is_sent = Feedback.objects.filter(
		vacancy=vacancy,
		applicant=applicant
	).exists
...
def feedback_list(request):
	...
	feedbacks = Feedback.objects.all().select_related("vacancy").filter(applicant_id=applicant).order_by("-id")
	context = {
		'feedbacks': feedbacks,
		'is_applicant': is_applicant,
...
def employer_feedback_list(request):
	...
	feedbacks = Feedback.objects.all().select_related("applicant").filter(vacancy__company=employer).order_by("-id")
	context = {
		'feedbacks': feedbacks,
		'is_applicant': is_applicant,
